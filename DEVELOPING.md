...
3. `RELOAD_MULTIKEY_CMD` is using to set multi-key command to reload the add-on's Nasal code. As default `:Yarnb`.
...
After changing these values, you need to reload the Nasal code using the "Dev Reload" menu item or the `:Yarnb` multi-key command, or, as a last resort, restart the entire simulator.
